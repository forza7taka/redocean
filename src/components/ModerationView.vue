<template>
  <v-card width="380px" class="mx-auto mt-5">
    <v-card-title>
      ...
      <v-card v-if="post" style="width: 380px" class="mx-auto mt-5">
        <v-card-actions>
      ...
      <v-card v-if="profile" width="380px" class="mx-auto mt-5" elevation="20">
        <v-card-actions>
          <v-list-item class="w-100">
            <template v-slot:prepend>
              <div style="padding-right: 10px">
    ...
    <v-card width="380px" class="mx-auto mt-5">
    <v-card-item>
      <v-select v-if="mode == 'post'" v-model="item" :hint="`${item.discription}`" :items="itemsForPost"
        item-title="name" item-value="value" label="report"
        persistent-hint return-object single-line></v-select>
      <v-select v-if="mode == 'user'" v-model="item" :hint="`${item.discription}`" :items="itemsForUser"
        item-title="name" item-value="value" label="report"
        persistent-hint return-object single-line></v-select>
    </v-card-item>
    <v-card-actions>
